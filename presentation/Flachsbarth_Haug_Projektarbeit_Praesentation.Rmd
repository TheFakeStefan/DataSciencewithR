...
germany <- germany %>% 
  mutate(LAND_NR = as.numeric(LAND_NR))
...
stopword_extented <- read_tsv("https://raw.githubusercontent.com/
solariz/german_stopwords/master/german_stopwords_full.txt", comment = ";")
...
```{r, eval=FALSE}
# Durchsuchen der Artikel nach jeder Partei
...
getSentiment <- function(text,jahr) {
  ...
  y <- ifelse(is.infinite(x_pos_score/x_neg_score),
